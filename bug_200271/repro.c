...
void execute_call(int call)
{
	...
	case 0:
		NONFAILING(memcpy((void*)0x20000080, "/dev/sg#", 9));
		res = syz_open_dev(0x20000080, 0, 0);
		if (res != -1)
				r[0] = res;
		break;
	case 1:
		NONFAILING(memcpy((void*)0x20000200, "\x01\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x00\xf6", 13));
		syscall(__NR_ioctl, r[0], 1, 0x20000200);
		break;
	case 2:
		NONFAILING(*(uint32_t*)0x20000040 = 0);
		NONFAILING(*(uint32_t*)0x20000044 = 1);
		NONFAILING(*(uint32_t*)0x200000c0 = 8);
		syscall(__NR_getsockopt, 0xffffff9c, 0x84, 0xd, 0x20000040, 0x200000c0);
		break;
	}
}
